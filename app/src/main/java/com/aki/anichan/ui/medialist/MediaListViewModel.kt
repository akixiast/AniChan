...
import com.aki.anichan.helper.enums.*
import com.aki.anichan.helper.pojo.MediaListAdapterComponent
...
class MediaListViewModel(
    ...
    fun selectSectionByName(name: String) {
        val groups = currentMediaListCollection?.lists ?: return
        val sections = ArrayList<String>()
        groups.forEach { sections.add(it.name) }
        if (isAllListPositionAtTop) sections.add(0, "All") else sections.add("All")
        // exact match first, then alias/prefix match (Watching<->Reading, Completed* splits)
        var index = sections.indexOfFirst { it.equals(name, ignoreCase = true) }
        if (index == -1) {
            val lower = name.lowercase()
            index = sections.indexOfFirst {
                val g = it.lowercase()
                when (lower) {
                    "watching" -> g == "reading" || g == "rewatching" || g == "rereading"
                    "completed" -> g.startsWith("completed")
                    "planning" -> g == "planning"
                    "dropped", "paused" -> g == lower
                    else -> false
                }
            }
        }
        if (index != -1) showSelectedSectionMediaList(index)
    }
    ...
    fun updateProgress(mediaList: MediaList, newProgress: Int, isProgressVolume: Boolean) {
        val currentProgress = when (mediaList.media.type) {
            com.aki.anichan.type.MediaType.ANIME -> mediaList.progress
            com.aki.anichan.type.MediaType.MANGA -> if (isProgressVolume) (mediaList.progressVolumes ?: 0) else mediaList.progress
            else -> 0
        }
        ...
        val maxProgress = when (mediaList.media.type) {
            com.aki.anichan.type.MediaType.ANIME -> mediaList.media.episodes
            com.aki.anichan.type.MediaType.MANGA -> if (isProgressVolume) mediaList.media.volumes else mediaList.media.chapters
            else -> null
        }

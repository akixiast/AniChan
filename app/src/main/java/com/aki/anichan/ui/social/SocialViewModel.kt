...
import com.aki.anichan.data.repository.SocialRepository
...
import com.aki.anichan.helper.pojo.SocialItem
        ...
        loadOnce {
            disposables.add(
                userRepository.getAppSetting()
                    .zipWith(userRepository.getViewer(Source.CACHE).onErrorReturnItem(User())) { appSetting, viewer ->
                        this.viewer = viewer
                    ...
                    .subscribe(
                        ...
                        {
                            // guest (no cached viewer): social needs login, show empty state
                            _adapterComponent.onNext(SocialAdapterComponent(viewer = User(), appSetting = com.aki.anichan.data.entity.AppSetting()))
                            state = State.ERROR
                        }
                    )
            )
        ...
        disposables.add(
            socialRepository.newOrEditedActivity
                ...
                .subscribe(
                    ...
                    {
                        // ignore activity-update stream errors, never crash
                    }
                )
        )
    }

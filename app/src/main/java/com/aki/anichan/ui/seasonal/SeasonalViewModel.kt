        ...
        loadOnce {
            disposables.add(
                Observable.zip(
                    ...
                    userRepository.getViewer(Source.CACHE).onErrorReturnItem(com.aki.anichan.data.response.anilist.User())
                ) { appSetting, user ->
                    appSetting to user
                }
                    .applyScheduler()
                    .subscribe(
                        ...
                        {
                            // guest (no cached viewer): fall back to defaults, no crash
                            _seasonalAdapterComponent.onNext(SeasonalAdapterComponent(ListType.LINEAR, appSetting))
                            loadSeasonal()
                        }
                    )
